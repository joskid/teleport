...
func TestCancelableChConn(t *testing.T) {
	...
	case sshConn := <-sshConnCh:
		...
		// Set the read deadline and make sure that the read above is
		// canceled with a timeout error within a second.
		chConn.SetReadDeadline(time.Now().Add(200 * time.Millisecond))
		select {
		case err := <-doneCh:
